...
TEST_F(S21MatrixTest, ConstructorMove) {
  S21Matrix src(*matrix_3x2);
  S21Matrix dest(std::move(src));
  EXPECT_EQ(0, src.GetRows());
  EXPECT_EQ(0, src.GetCols());
  EXPECT_EQ(3, dest.GetRows());
  EXPECT_EQ(2, dest.GetCols());
  EXPECT_EQ(true, dest == *matrix_3x2);
}
...
TEST_F(S21MatrixTest, OperatorSum1) {
  S21Matrix matrixA(*matrix_3x3), expected(3, 3);
  expected(0, 0) = 4;
  expected(0, 1) = 10;
  expected(0, 2) = 14;
  expected(1, 0) = 12;
...
TEST_F(S21MatrixTest, OperatorAssignIndexes1) {
  S21Matrix matrixA;
  EXPECT_DOUBLE_EQ(0, matrixA(0, 0));
  matrixA(0, 0) = -4;
  EXPECT_DOUBLE_EQ(-4, matrixA(0, 0));
}
